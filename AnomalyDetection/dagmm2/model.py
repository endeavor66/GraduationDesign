...
import itertools
from AnomalyDetection.dagmm2.utils import *
...
class DaGMM(nn.Module):
    ...
    def __init__(self, n_gmm = 2, latent_dim=3):
        ...
        #压缩网络 - 降维
        layers = []
        layers += [nn.Linear(len(Config.train_col), 6)]
        layers += [nn.Tanh()]        
        ...
        #压缩网络 - 还原
        layers = []
        layers += [nn.Linear(1,3)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(3,6)]
        layers += [nn.Tanh()]        
        layers += [nn.Linear(6, len(Config.train_col))]
